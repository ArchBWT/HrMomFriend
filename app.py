...
class ResumeApp:
    ...
    def render_builder(self):
        st.title("Сформировать Резюме")
        st.write("Создайте свое профессиональное резюме")

        # Template selection
        template_options = ["Современный", "Профессиональный", "Минимальный", "Креативный"]
        selected_template = st.selectbox(
    "Выберите шаблон резюме", template_options)
        st.success(f"🎨 Сейчас выбран: {selected_template} Шаблон")

        # Personal Information
        st.subheader("Персональная Информация")
        ...
        with col1:
            ...
            full_name = st.text_input("Полное Имя", value=existing_name)
            email = st.text_input(
    "Email",
    value=existing_email,
     key="email_input")
            phone = st.text_input("Номер Телефона", value=existing_phone)
        ...
        with col2:
            ...
            # Input fields with existing values
            location = st.text_input("Город", value=existing_location)
            linkedin = st.text_input("HH Портфолио", value=existing_linkedin)
            portfolio = st.text_input(
    "Веб-сайт портфолио", value=existing_portfolio)
        ...
        st.subheader("Профессиональное резюме")
        summary = st.text_area("Расскажите о себе", value=st.session_state.form_data.get('summary', ''), height=150,
                             help="Напишите краткое резюме, подчеркнув ваши основные навыки и опыт.")
        ...
        for idx, exp in enumerate(st.session_state.form_data['experiences']):
            with st.expander(f"Опыт {idx + 1}", expanded=True):
                col1, col2 = st.columns(2)
                with col1:
    ...
    "Позиция", key=f"position_{idx}", value=exp.get(
        'position', ''))
                with col2:
                    exp['start_date'] = st.text_input(
    "Дата Приема", key=f"start_date_{idx}", value=exp.get(
        'start_date', ''))
                    exp['end_date'] = st.text_input(
    "Дата Увольнения", key=f"end_date_{idx}", value=exp.get(
        'end_date', ''))

                exp['description'] = st.text_area("Какие роли занимали", key=f"desc_{idx}",
                                                value=exp.get(
                                                    'description', ''),
                                                help="Краткий обзор вашей роли")

                # Responsibilities
                st.markdown("##### Обязанности")
                resp_text = st.text_area("Введите обязанности (по одной в каждой строке)",
                                       key=f"resp_{idx}",
                                       value='\n'.join(
                                           exp.get('responsibilities', [])),
                                       height=100,
                                       help="Введите обязанности (по одному в каждой строке)")
                exp['responsibilities'] = [r.strip()
                                                   for r in resp_text.split('\n') if r.strip()]

                # Achievements
                st.markdown("##### Ключевые достижения")
                achv_text = st.text_area("Введите достижения (по одному в каждой строке)",
                                       key=f"achv_{idx}",
                                       value='\n'.join(
                ...
                if st.button("Удалить", key=f"remove_exp_{idx}"):
        ...
        for idx, proj in enumerate(st.session_state.form_data['projects']):
            with st.expander(f"Проект {idx + 1}", expanded=True):
    ...
    value=proj.get(
        'name',
         ''))
                proj['technologies'] = st.text_input("Используемые технологии", key=f"proj_tech_{idx}",
                                                   value=proj.get(
                                                       'technologies', ''),
                                                   help="Перечислите основные используемые технологии, фреймворки и инструменты.")

                proj['description'] = st.text_area("Обзор проекта", key=f"proj_desc_{idx}",
                                                 value=proj.get(
                                                     'description', ''),
                                                 help="Краткий обзор проекта и его целей")

                # Project Responsibilities
                st.markdown("##### Обязанности")
                proj_resp_text = st.text_area("Введите обязанности (по одной в каждой строке)",
                                            key=f"proj_resp_{idx}",
                                            value='\n'.join(
                                                proj.get('responsibilities', [])),
                                            height=100,
                                            help="Перечислите ваши основные обязанности в проекте")
                proj['responsibilities'] = [r.strip()
                                                    for r in proj_resp_text.split('\n') if r.strip()]

                # Project Achievements
                st.markdown("##### Ключевые достижения")
                proj_achv_text = st.text_area("Введите достижения (по одному в каждой строке)",
                                            key=f"proj_achv_{idx}",
                                            value='\n'.join(
                                                proj.get('achievements', [])),
                                            height=100,
                                            help="Перечислите основные достижения проекта и ваш вклад.")
                proj['achievements'] = [a.strip()
                                                for a in proj_achv_text.split('\n') if a.strip()]

                proj['link'] = st.text_input("Ссылка на проект (необязательно)", key=f"proj_link_{idx}",
                                           value=proj.get('link', ''),
                                           help="Ссылка на репозиторий проекта, демо-версию или документацию")

                if st.button("Удалить проект", key=f"remove_proj_{idx}"):
        ...
        if st.button("Добавить образование"):
        ...
        for idx, edu in enumerate(st.session_state.form_data['education']):
            with st.expander(f"Образование {idx + 1}", expanded=True):
                col1, col2 = st.columns(2)
                with col1:
    ...
    "Степень", key=f"degree_{idx}", value=edu.get(
        'degree', ''))
                with col2:
    ...
    value=edu.get(
        'gpa',
         ''))
                ...
                st.markdown("##### Достижения и участие в проектах")
                edu_achv_text = st.text_area("Введите достижения (по одному в строке)",
                                           key=f"edu_achv_{idx}",
                                           value='\n'.join(
                                               edu.get('achievements', [])),
                                           height=100,
                                           help="Укажите академические достижения, соответствующие курсы или виды деятельности")
                edu['achievements'] = [a.strip()
                                               for a in edu_achv_text.split('\n') if a.strip()]

                if st.button("Удалить образование", key=f"remove_edu_{idx}"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            tech_skills = st.text_area("Технические навыки (по одному в строке)",
                                     value='\n'.join(
    st.session_state.form_data['skills_categories']['technical']),
                                     height=150,
                                     help="Языки программирования, фреймворки, базы данных и др.")
            st.session_state.form_data['skills_categories']['technical'] = [
                s.strip() for s in tech_skills.split('\n') if s.strip()]

            soft_skills = st.text_area("Гибкие/социальные навыки (по одному в строке)",
                                     value='\n'.join(
    st.session_state.form_data['skills_categories']['soft']),
                                     height=150,
                                     help="Лидерство, коммуникация, решение проблем и др.")
            st.session_state.form_data['skills_categories']['soft'] = [
                s.strip() for s in soft_skills.split('\n') if s.strip()]

        with col2:
            languages = st.text_area("Языки (по одному в строке)",
                                   value='\n'.join(
    st.session_state.form_data['skills_categories']['languages']),
                                   height=150,
                                   help="Языки программирования или иностранные языки с указанием уровня владения")
            st.session_state.form_data['skills_categories']['languages'] = [
                l.strip() for l in languages.split('\n') if l.strip()]

            tools = st.text_area("Инструменты и технологии (по одному в строке)",
                               value='\n'.join(
    st.session_state.form_data['skills_categories']['tools']),
                               height=150,
                               help="Инструменты разработки, программное обеспечение, платформы и др.")
            st.session_state.form_data['skills_categories']['tools'] = [
        ...
        if st.button("Создать резюме 📄", type="primary"):
    ...
    f"Email input value: {
        st.session_state.get(
            ...
            if not current_name:
                st.error("⚠️ Пожалуйста, введите ваше полное имя.")
                return

            if not current_email:
                st.error("⚠️ Пожалуйста, введите ваш адрес электронной почты.")
                return
            ...
            try:
                print("Подготовка данных для резюме…")
                ...
                try:
                    ...
                    if resume_buffer:
                        try:
                            ...
                            st.success("✅ Резюме успешно создано!")
    ...
    def render_analyzer(self):
        ...
        page_header(
            "Анализатор резюме",
            "Получите мгновенную обратную связь на основе искусственного интеллекта для оптимизации вашего резюме"
        )
    ...
    "Категория", categories, key="standard_category")

            roles = list(self.job_roles[selected_category].keys())
            selected_role = st.selectbox(
    "Текущий Стек", roles, key="standard_role")
        ...
        'pdf', 'docx'], key="standard_file")
            ...
            if not uploaded_file:
                ...
                st.markdown(
                    ...
                    "Загрузите свое резюме, чтобы начать анализ"
                    ),
                    unsafe_allow_html=True
                )
            ...
            if uploaded_file:
                ...
                analyze_standard = st.button("🔍 Проанализируй мое резюме",
                                    type="primary",
    ...
    "</ul></div>", unsafe_allow_html=True)
        ...
        with analyzer_tabs[1]:
            st.markdown("""
            <div style='background-color: #1e1e1e; padding: 20px; border-radius: 10px; margin: 10px 0;'>
                <h3>Анализ резюме с помощью ИИ</h3>
                <p>Получите подробную информацию от продвинутых моделей искусственного интеллекта, которые анализируют ваше резюме и предоставляют персонализированную рекомендацию.</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    text="Количество резюме", font=dict(
        size=16, color="white")),
            ...
            categories = list(self.job_roles.keys())
            selected_category = st.selectbox(
    "Категория", categories, key="ai_category")

            roles = list(self.job_roles[selected_category].keys())
            selected_role = st.selectbox("Текущий Стек", roles, key="ai_role")
    ...
    "Загрузите свое резюме", type=[
        'pdf', 'docx'], key="ai_file")
            ...
            # Display empty state with a prominent upload button
                ...
                self.render_empty_state(
            "fas fa-robot",
                        "Загрузите свое резюме, чтобы получить анализ и рекомендации на основе искусственного интеллекта"
        ),
        unsafe_allow_html=True
    )
            else:
    ...
    def render_home(self):
        ...
        # Hero Section
        hero_section(
            "Анализатор Резюме с использованием ИИ"
        )
        ...
        feature_card(
            "fas fa-robot",
            "Анализ на основе ИИ",
            "Получите мгновенную обратную связь по своему резюме с помощью расширенного анализа на основе искусственного интеллекта."
        )
        
        feature_card(
            "fas fa-magic",
            "Умный конструктор резюме",
            "Создавайте профессиональные резюме с помощью интеллектуального конструктора."
        )
        ...
        with col2:
            if st.button("Начать", key="get_started_btn", 
    ...
    def render_feedback_page(self):
        ...
        page_header(
            "Обратнная связь",
            "Дорогие тестировщики, помогите стать проекту лучше"
        )
        ...
        form_tab, stats_tab = st.tabs(["Написать Отзыв", "Статистика Отзывов"])
