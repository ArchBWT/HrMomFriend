...
class FeedbackManager:
    ...
    def render_feedback_form(self):
        ...
        st.markdown('<h2 class="feedback-header">📝 Поделитесь своим опытом!</h2>', unsafe_allow_html=True)

        # Overall Rating
        st.markdown('<div class="feedback-section">', unsafe_allow_html=True)
        st.markdown('<label class="feedback-label">Общая оценка</label>', unsafe_allow_html=True)
        ...
        # Usability Score
        st.markdown('<div class="feedback-section">', unsafe_allow_html=True)
        st.markdown('<label class="feedback-label">Насколько было легко пользоваться?</label>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="feedback-section">', unsafe_allow_html=True)
        st.markdown('<label class="feedback-label">Насколько вы удовлетворены функционалом?</label>', unsafe_allow_html=True)
        ...
        # Text Feedback
        st.markdown('<div class="feedback-section">', unsafe_allow_html=True)
        st.markdown('<label class="feedback-label">Какие функции вы хотели бы хотели видеть?</label>', unsafe_allow_html=True)
        missing_features = st.text_area("Missing Features", placeholder="Поделитесь своими пожеланиями по функционалу...", label_visibility="collapsed")

        st.markdown('<label class="feedback-label">Что мы можем улучшить?</label>', unsafe_allow_html=True)
        improvement_suggestions = st.text_area("Improvement Suggestions", placeholder="Ваши предложения по улучшению...", label_visibility="collapsed")

        st.markdown('<label class="feedback-label">Расскажите нам о своем опыте</label>', unsafe_allow_html=True)
        user_experience = st.text_area("User Experience", placeholder="Поделитесь с нами своим опытом...", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

        # Submit Button
        if st.button("Отправить Отзыв", key="submit_feedback"):
            try:
                # Create progress bar
    ...
    def render_feedback_stats(self):
        ...
        st.markdown("""
            <div style="text-align: center; padding: 15px; background: linear-gradient(90deg, rgba(76, 175, 80, 0.1), rgba(33, 150, 243, 0.1)); border-radius: 10px; margin-bottom: 20px;">
                <h3 style="color: #E0E0E0;">Статистика 📊</h3>
            </div>
        """, unsafe_allow_html=True)
